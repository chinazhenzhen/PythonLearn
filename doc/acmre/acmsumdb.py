...
class ACMREdb:
    ...
    def querydbAll(self):
        sql=""" SELECT * FROM acmsolvesum ORDER BY codeforcerank DESC"""
        self.cur.execute(sql)
    ...
    def querydbSome(self,tag):
        sql=""" SELECT * FROM acmsolvesum WHERE  id REGEXP'%s' ORDER BY codeforcerank DESC"""%(tag)
        self.cur.execute(sql)
